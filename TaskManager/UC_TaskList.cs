...
namespace TaskManager
{
    public partial class UC_TaskList : UserControl, ICloseWindow
    {
        ...
        void QuartzHelper_OnTriggerComplete(Quartz.ITrigger trigger, Quartz.IJobExecutionContext context, Quartz.SchedulerInstruction triggerInstructionCode)
        {
            ...
            DateTime nextRuntime = TimeZoneInfo.ConvertTimeFromUtc(context.NextFireTimeUtc.Value.DateTime, TimeZoneInfo.Local);
        ...
        bool QuartzHelper_OnVetoJobExecution(Quartz.ITrigger trigger, Quartz.IJobExecutionContext context)
        {
            ...
            //TaskUtil.UpdateRecentRunTime(taskid, TimeZoneInfo.ConvertTimeFromUtc(context.NextFireTimeUtc.Value.DateTime, TimeZoneInfo.Local));
        ...
        public void AddItem(TaskUtil task)
        {
            ...
            //item.SubItems.Add(task.LastRunTime.ToString("yyyy-MM-dd hh:mm:ss"));
            item.SubItems.Add("");
        ...
        private void tlbtn_New_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (fmtask.ShowDialog() == DialogResult.OK)
                {
                    ...
                    {
                        AddItem(fmtask.Task);
                        QuartzHelper.ScheduleJob(fmtask.Task);
                    }
                }
            }
            catch (Exception er)
        ...
        private void tlbtn_Edit_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.listView1.SelectedItems.Count > 0)
                {
                    ...
                    if (task != null)
                    {
                        ...
                        if (fmtask.ShowDialog() == DialogResult.OK)
                        {
                            ...
                            QuartzHelper.ScheduleJob(task, true);
                        }
                    }
                }
            }
        ...
        private void tlbtn_Delete_Click(object sender, EventArgs e)
        {
            if (this.listView1.SelectedItems.Count > 0)
            {
                ListViewItem item = this.listView1.SelectedItems[0];
                ...
                if (task != null)
                {
                    if (MessageService.AskQuestion("您要删除此任务吗？"))
                    {
                        TaskUtilService.Instance.Delete(task);
                        QuartzHelper.DeleteJob(task.TaskID.ToString());
                        lock (rootlock)
                        {
                            item.Remove();
                        }
        ...
        private void tlbtn_Stop_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.listView1.SelectedItems.Count > 0)
                {
                    ...
                    if (task != null)
                    {
                        ...
                        //task["LastRunTime"] = System.DateTime.Now.ToString("yyyy-MM-dd");
                        //task.SaveChanges();
                        TaskUtilService.Instance.SaveChanges(task);
                        QuartzHelper.ScheduleJob(task, true);
                        UpdateItem(item, task, 0);
        ...
        private void tlbtn_Reload_Click(object sender, EventArgs e)
        {
            try
            {
                LoadData();
                QuartzHelper.Refresh();
            }
            catch (Exception ex)
            {
        ...
        private void tlstripbtn_EXENow_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.listView1.SelectedItems.Count > 0)
        ...
        public void Close()
        {
            QuartzHelper.OnVetoJobExecution -= QuartzHelper_OnVetoJobExecution;
            QuartzHelper.OnTriggerComplete -= QuartzHelper_OnTriggerComplete;
            QuartzHelper.StopSchedule(false);
        }
    }
}
